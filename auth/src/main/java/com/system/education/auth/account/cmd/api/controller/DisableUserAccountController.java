...
@RequestMapping(path = "/api/v1/education/user-account/disable-user-account")
...
public class DisableUserAccountController {
    ...
    public Mono<ResponseEntity<?>> disable (@Valid @RequestBody ChangeUserStatusCommand request) {
        return userAccountEventHandler.disable(request)
                .flatMap(userAccount -> {
                    if (userAccount != null) {
                        return Mono.just(ResponseEntity.ok(new MessageResponse(true, UserAccountUtilsConstants.disable)));
                    } else {
