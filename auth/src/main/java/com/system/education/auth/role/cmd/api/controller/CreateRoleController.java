...
@RequestMapping(path = "/api/v1/education/role/create-role")
...
public class CreateRoleController {
    ...
    public Mono<ResponseEntity<?>> create(@Valid @RequestBody RoleCreatedRequest request) {
        return rolePayload.exception(request)
                .flatMap(messageResponse -> {
                    ...
                    return roleEventHandler.create(request)
                            .flatMap(role -> {
                                if (role != null) {
