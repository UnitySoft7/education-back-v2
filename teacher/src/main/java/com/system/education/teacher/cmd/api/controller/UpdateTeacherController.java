...
@RequestMapping(path = "/api/v1/education/teacher/update-teacher")
...
public class UpdateTeacherController {
    ...
    /**
     * This method is used to update a teacher
     * @param command the command containing the teacher information
     * @return the message for operation
     */
    @Operation(summary = "Update teacher")
    @PutMapping(produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseEntity<MessageResponse>> update (@Valid @RequestBody TeacherUpdatedCommand command) {
        return teacherPayload.isTeacherCodeExist(command.teacherCode())
                .flatMap(isExists -> {
                    if (isExists) {
                        return teacherEventHandler.update(command)
                                .flatMap(teacher -> {
                                    if (teacher != null) {
