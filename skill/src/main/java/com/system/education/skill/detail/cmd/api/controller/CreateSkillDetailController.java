...
@RequestMapping(path = "/api/v1/education/skill/create-skill-detail")
@Tag(name = "Skill-detail", description = "Data rest API for skill-detail resource")
public class CreateSkillDetailController {
    private final SkillDetailEventHandler skillDetailEventHandler;

    /**
     * This method is used to create a new skill-detail
     * @param command the command containing the skill-detail information
     * @return the message for operation
     */
    @Operation(summary = "Create skill-detail")
    @PostMapping(produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseEntity<MessageResponse>> create (@Valid @RequestBody SkillDetailCreatedCommand command) {
        return skillDetailEventHandler.create(command)
                .flatMap(skillDetail -> {
                    if (skillDetail != null) {
