...
public class EnableEstablishmentController {
    ...
    public Mono<ResponseEntity<?>> enable (@Valid @RequestBody ChangeEstablishmentStateCommand command) {
        return establishmentPayload.isEstablishmentCodeExist(command.establishmentCode())
                .flatMap(exist -> {
                    if (exist) {
                        return establishmentPayload.isEnableEstablishment(command.establishmentCode()).flatMap(enable -> {
                            if (enable) {
                                return Mono.just(ResponseEntity.ok()
                                        .body(new MessageResponse(false, EstablishmentUtilsConstants.establishment_enable_exception)));
                            }
                            Mono<Establishment> establishmentMono = establishmentEventHandler.enable(command);
                            return establishmentMono.flatMap(establishment -> {
                                if (establishment != null) {
                                    return Mono.just(ResponseEntity.ok()
                                            .body(new MessageResponse(true,EstablishmentUtilsConstants.establishment_enable)));
                                }
                                return Mono.just(ResponseEntity.ok()
