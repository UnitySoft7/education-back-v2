package com.system.education.infirmary.consumed.query.api.controller;

import com.system.education.infirmary.consumed.query.api.dto.AllLookupConsumedResponse;
import com.system.education.infirmary.consumed.query.api.dto.LookupConsumedResponse;
...
@RequestMapping(path = "/api/v1/education/infirmary/consumed-lookup/")
...
public class ConsumedLookupController {
     ...
     * @return the consumed with the specified code and semester and status
     */
    @Operation(summary = "Retrieve data consumed by diagnosis and status")
    @PutMapping(path = "get-consumed-by-diagnosis", produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseEntity<?>> getByDiagnosis(@Valid @RequestBody ConsumedByDiagnosisCodeQuery query) {
        return consumedQueryHandler.getConsumedByDiagnosis(query)
                .collectList()
                .map(consumed ->
                        new AllLookupConsumedResponse(true, consumed))
                .map(ResponseEntity::ok);
