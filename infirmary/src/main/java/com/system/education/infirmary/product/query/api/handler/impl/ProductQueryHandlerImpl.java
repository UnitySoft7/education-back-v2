...
public class ProductQueryHandlerImpl implements ProductQueryHandler {
     ...
     * @param infirmaryProduct the product to convert
     * @return the driver response
     */
    private Mono<ProductResponse> getProductResponse(InfirmaryProduct infirmaryProduct) {
        return Mono.just(
                new ProductResponse(infirmaryProduct.getProductId(),
                        infirmaryProduct.getProductCode(), infirmaryProduct.getProductName(),
                        infirmaryProduct.getAmount(), infirmaryProduct.getEstablishmentCode(),
                        infirmaryProduct.getEstablishmentName())
        );
    }
}
