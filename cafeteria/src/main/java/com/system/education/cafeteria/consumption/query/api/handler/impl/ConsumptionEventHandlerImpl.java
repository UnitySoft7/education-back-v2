...
public class ConsumptionEventHandlerImpl implements ConsumptionEventHandler {
                ...
                .flatMap(isBoolean -> {
                    if (isBoolean) {
                        return consumptionRepository.findByStudentCodeAndSemesterAndSchoolYear(command.studentCode(), command.semester(), command.schoolYear())
                                .flatMap(consumption -> {
                                    consumption.setConsumptionAmount(consumption.getConsumptionAmount() + command.amount());
                                    consumption.setStatus(Status.not_complete());
    ...
    public Mono<MessageResponse> pay(ConsumptionPayCommand command) {
        return consumptionRepository.findByStudentCodeAndSemesterAndSchoolYear(command.studentCode(), command.semester(), command.schoolYear())
                .flatMap(consumption -> {
                    double amount = consumption.getPayedAmount() + command.amount();
                    consumption.setPayedAmount(amount);
                    if (consumption.getConsumptionAmount() <= amount)
                        consumption.setStatus(Status.complete());
                    return consumptionRepository.save(consumption);
