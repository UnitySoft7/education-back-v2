...
@RequestMapping(path = "/api/v1/education/cafeteria/pay-all-consumed")
@Tag(name = "Consumed", description = "Data rest API for consumed resource")
public class PayAllProductController {
    private final ConsumedEventHandler consumedEventHandler;

    /**
     * This method is used to pay all consumed
     * @param command the command containing the consumed information
     * @return the message for operation
     */
    @Operation(summary = "Pay all consumed")
    @PostMapping(produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseEntity<MessageResponse>> create (@Valid @RequestBody PayAllProductCommand command) {
        return consumedEventHandler.payAllProduct(command)
                .flatMap(consumed -> {
                    if (consumed != null) {
